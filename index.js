...
function processSum(numberList, callback) {
  return (numberList.length > 0) ? callback(numberList.reduce((accumulator, currentValue) => accumulator + currentValue)):0 + 1000;
}
...
function getFullNames(runners) {
  ...
  runners.forEach(function(runner, idx) {
    lastCommaFirst.push(`${runner.last_name}, ${runner.first_name}`);
  });
...
function firstNamesAllCaps(runners) {
  return runners.map(function(runner, idx) {
